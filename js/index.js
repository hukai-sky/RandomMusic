    ...
    $.post('http://api.jirengu.com/fm/getLyric.php', {ssid: sSid, sid: sid})
        .done(function (lyr){
            ...
            if (lyr.lyric) {
                ...
                var lyArr = lyr.lyric.split('\n');//歌词以排数为界的数组
                var timeReg = /\[\d{2}:\d{2}.\d{2}\]/g;//时间的正则
                var result = [];
                if(lyArr !== ""){
                    for(var i in lyArr){//遍历歌词数组
                        var time = lyArr[i].match(timeReg);//每组匹配时间 得到时间数组
                        if(!time)continue;//如果没有 就跳过继续
                        var value = lyArr[i].replace(timeReg,"");// 纯歌词
        ...
        $('.music-lyric .lyric').html("<li>本歌曲暂时没有歌词</li>");
    })
}
...
function showLyric(){
    ...
    for(var i=0;i< lyricArr.length;i++){//遍历歌词下所有的li
        ...
        if ((curTime > curT) && (curT < nexT)){//当前时间在下一句时间和歌曲当前时间之间的时候 就渲染 并滚动
            ...
            $('.music-lyric .lyric').css('top', -liH*(i-7));
        }
    }
...
$(".basebar").mousedown(function(ev){  //拖拽进度条控制进度
    ...
    var percentage = (posX - targetLeft)/350*100;
